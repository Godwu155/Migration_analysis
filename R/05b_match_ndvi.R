...
ndvi_path <- file.path(
  ...
  "data",
  "raw",
  "CURLEW_VLAANDEREN - Eurasian curlews (Numenius arquata, Scolopacidae)NVDI.csv"
)
...
ndvi_col <- "MODIS Land Vegetation Indices 1km Monthly Terra NDVI"
...
ndvi <- ndvi_raw |>
  transmute(
    ...
    ts_ndvi = safe_time(timestamp),
    ndvi_sec = as.numeric(ts_ndvi),
    lon_ndvi = `location-long`,
    lat_ndvi = `location-lat`,
